    ...
    for (var i = 0; i < state.cartData.length; i++) {
      ...
      if (state.cartData[i].atribute) {
        addValue = {
          ...
          atributesValue: getAtributeProductValue(
            state.cartData[i].atributes,
...
const calculateProgressiveDiscount = function(state, getters) {
  ...
  if (state.paymentRules.length > 0) {
    let rules = state.paymentRules.filter(
      (a) => a.application_type_rules !== null
    );
    ...
    if (payRules) {
      if (payRules.application_type_rules) {
        ...
        if (payRules.rules.length > 0) {
          for (let i = 0; i < payRules.rules.length; i++) {
            if (
              ...
              payRules.rules[i].amount <=
                getters.getProductsWithoutSale.totalAmount
            ) {
              discountRate = payRules.rules[i].rate;
            }
          }
        ...
        return {
          ...
          discountValue:
            getters.getProductsWithoutSale.totalPrice > 0
              ? getters.getProductsWithoutSale.totalPrice * (discountRate / 100)
              : discountRate,
        };
      }
    }
...
const getProductsWithoutSale = function(state, getters) {
  ...
  for (let i = 0; i < getters.productsWithImgUrl.length; i++) {
    if (getters.productsWithImgUrl[i].actived_promotions.length <= 0) {
      itens.push({
        price:
          getters.productsWithImgUrl[i].price *
          getters.productsWithImgUrl[i].amount,
        amount: getters.productsWithImgUrl[i].amount,
      });
  ...
  return { itens: itens, totalAmount: totalAmount, totalPrice: totalPrice, totalItems: totalItems };
};
...
const totalProductsValue = function(state) {
  ...
  for (let i = 0; i < state.cartData.length; ++i) {
    if (state.cartData[i].atributes) {
      response =
        response +
        getAtributeProductValue(
          state.cartData[i].atributes,
          state.cartData[i].amount
        ) +
        parseFloat(state.cartData[i].price) * state.cartData[i].amount;
    } else {
      response =
        response +
        parseFloat(state.cartData[i].price) * state.cartData[i].amount;
