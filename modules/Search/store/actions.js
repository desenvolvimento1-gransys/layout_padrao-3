...
const callSearch = function ({ commit }, payload) {
 ...
 const fetchProducts = function ({ commit }) {
  $nuxt.$nextTick(() => {
    $nuxt.$loading.start();
  });
  ...
  axios.post(`${baseApiUrl}/ecommerce/search/default-search`, payload)
    .then(res => {
      console.log("Resposta da API:", res.data); // Debug para verificar os dados retornados
      commit('SET_LIST', res.data.data); // Armazena os dados no estado Vuex
