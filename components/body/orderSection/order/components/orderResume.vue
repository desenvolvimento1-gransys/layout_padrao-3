...
export default {
  ...
  methods: {
    ...
    sendPurchaseEvent(items) {
      const checkoutItems = items.map(item => ({
        item_id: item.item_id || `${this.$route.params.orderId}`,
        item_name: item.name,
        price: item.total_price,
        quantity: item.amount,
        Rota: `${this.$nuxt.$route.path}`,
        discount: item.discount_value || 0,
      }));

      // Dispara a nova ação
      this.$store.dispatch('GoogleTagEvents/storeAndDispatchPurchase', { items: checkoutItems });

      console.log('Evento storeAndDispatchPurchase disparado com os seguintes itens:', checkoutItems);
    },




    sendCheckoutEvent(items) {
      const checkoutItems = items.map(item => ({
        item_id: `${this.$route.params.orderId}`,
        item_name: item.name,
        price: item.total_price,
        quantity: item.amount,
      ...
      this.$store.dispatch('GoogleTagEvents/begin_checkout', { items: checkoutItems });
      console.log('Evento begin_checkout disparado com os seguintes itens:', checkoutItems);
    },

  },
  mounted() {
    console.log('Componente montado - verificando dados...');
    this.setOrderResume({ buy_id: this.buy_id });
  },
  watch: {
    resumeList(newVal) {
      console.log('resumeList atualizado:', newVal);
      if (newVal && newVal.length > 0) {
        this.sendCheckoutEvent(newVal); // Dispara o evento para todos os itens
        this.sendPurchaseEvent(newVal); // Dispara o evento para todos os itens
      } else {
        console.warn('resumeList ainda está vazio.');
      }
    }
  },
