...
export default {
  ...
  methods: {
    ...
    SendDataViewCart: function(){
      if (this.totalProductsValue != 0 && this.productsWithImgUrl.length > 0) {
        let ProductMoreDelivery = parseFloat(this.totalProductsValue) + parseFloat(this.deliveryPrice);

        let SendViewCart = {
          value: `${parseFloat(ProductMoreDelivery).toFixed(2)}`,
          items: this.productsWithImgUrl.map(item => ({
            item_name: item.name,
            item_id: item.id,
            total_price: `${(item.amount * item.price).toFixed(2)}`,
            quantity:`${item.amount}`,
            discount: `${this.$route.params.progressiveDiscount || ''}`
          })),
        };

        if (SendViewCart.items && SendViewCart.items.length > 0) {
          $nuxt.$store.dispatch("GoogleTagEvents/view_cart", SendViewCart);
        } else {
          console.error('No products found in cart for GTM event');
        }
      }
    },
